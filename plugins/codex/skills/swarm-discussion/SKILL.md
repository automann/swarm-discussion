...
| `collectResult(...)` (a step's batch) | `multi_agent_v1.wait_agent`, then `python3 collect.py --spawn-order '<[{agentId,persona,token}]>' < wait_result.json` → results in spawn order. **`wait_agent` may return a subset of completed targets even with `timed_out:false`** — poll the remaining agents, accumulate completed statuses into one map, THEN demux. `collect.py` `errors` for a not-yet-complete agent mean *poll again*; `errors`/`timedOut` after all targets are done ⇒ re-spawn. |
...
4. `wait_agent` (poll + accumulate until every spawn-order agent is present) → `collect.py` demux → results in
